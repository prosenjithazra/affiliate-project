...
import {
  Search, Sun, Moon, Heart,
  Laptop, Shirt, Sparkles, Gamepad2, Dumbbell,
  Compass, ShoppingBag, Home, Grid3X3, Flame,
  ChevronDown, Phone, List, BookOpen,
} from "lucide-react";
import { Category } from "@repo/types";

const getIcon = (iconName: string, cls = "h-4 w-4") => {
  const map: Record<string, React.ReactNode> = {
    Laptop: <Laptop className={cls} />,
    Shirt: <Shirt className={cls} />,
    Sparkles: <Sparkles className={cls} />,
    Gamepad2: <Gamepad2 className={cls} />,
    Dumbbell: <Dumbbell className={cls} />,
    Compass: <Compass className={cls} />,
  };
  return map[iconName] || <ShoppingBag className={cls} />;
};
    ...
    <>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4 lg:gap-6 py-2.5 sm:min-h-16">
            ...
            <div className="flex items-center justify-between w-full sm:w-auto shrink-0">
              {/* Logo */}
              <Link href="/" className="flex items-center shrink-0 min-w-[116px]">
                <img
            ...
            <form onSubmit={handleSearch} className="hidden sm:block w-full sm:flex-1 sm:max-w-2xl sm:mx-auto min-w-0">
              <div
        ...
        <div className="hidden sm:block border-t border-slate-100 dark:border-slate-900 bg-blue-700 dark:bg-slate-900/90 shadow-sm">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-wrap items-center gap-x-2 min-h-11 overflow-visible">
            ...
            <div className="hidden xl:flex items-center gap-1.5 text-xs font-bold text-white/90 shrink-0 ml-auto">
              ...
              <span>Hotline: <strong className="text-white">+1-800-SHOPZONE</strong></span>
            </div>
        ...
        <div className="sm:hidden border-b border-blue-850 bg-blue-700 shadow-sm">
          <div className="flex gap-2 overflow-x-auto no-scrollbar px-3 py-2">
            {categories.map((cat) => (
              ...
              >
                <span className="text-orange-300">{getIcon(cat.icon, "h-3.5 w-3.5")}</span>
                <span className="capitalize">{cat.name}</span>
              </Link>
            ))}
            <Link
      ...
      <nav className="sm:hidden fixed bottom-0 inset-x-0 z-50 border-t border-slate-200/60 dark:border-slate-800 bg-white/95 dark:bg-slate-950/95 backdrop-blur-md supports-[backdrop-filter]:bg-white/85 dark:supports-[backdrop-filter]:bg-slate-950/85 shadow-lg">
        <div className="grid grid-cols-4 h-[calc(4rem+env(safe-area-inset-bottom))] pb-[env(safe-area-inset-bottom)]">
          {[
            { href: "/", label: "Home", icon: <Home className="h-4.5 w-4.5" /> },
            { href: "/search", label: "Catalog", icon: <Grid3X3 className="h-4.5 w-4.5" /> },
            { href: "/deals", label: "Deals", icon: <Flame className="h-4.5 w-4.5" /> },
            { href: "/blog", label: "Blog", icon: <BookOpen className="h-4.5 w-4.5" /> },
          ].map((item) => (
            <Link
              key={item.label}
              href={item.href}
            ...
            >
              <span className="text-orange-600 dark:text-orange-500">{item.icon}</span>
              {item.label}
            </Link>
          ))}
        </div>
